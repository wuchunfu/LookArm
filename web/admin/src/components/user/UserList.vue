        ...
        user_name: [
          {
            validator: (rule, value, callback) => {
              ...
              if ([...this.userInfo.user_name].length < 4 || [...this.userInfo.user_name].length > 12) {
        ...
        user_name: [
          {
            validator: (rule, value, callback) => {
              ...
              if ([...this.newUser.user_name].length < 4 || [...this.newUser.user_name].length > 12) {
                callback(new Error('用户名应当在4到12个字符之间'))
              } else {
                callback()
              }
  ...
  methods: {
    ...
    deleteUser(id) {
      this.$confirm({
        ...
        onOk: async () => {
          const { data: res } = await this.$http.delete(`delete_user/${id}`)
