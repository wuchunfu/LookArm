<template>
  <div class="mt-2" app>
    <div v-if="total == 0 && isLoad" class="d-flex justify-center align-center">
      <div>
        <v-alert class="ma-5" dense outlined type="error">抱歉，你搜索的APP还未搜录，提交个表单告诉我们吧！</v-alert>
      </div>
    </div>
    ...
    <v-row justify="center" align="center">
      <v-col cols="6">
        <v-card outlined elevation="2" class="my-4" v-for="item in appInfoList" :key="item.id">
          ...
          <v-card-subtitle class="font-weight-bold">简短描述：{{item.app_desc}}</v-card-subtitle>
          ...
          <v-card-actions>
            <v-row align="center" justify="center">
              ...
              <v-col cols="4">
                <v-btn text>本条信息由{{item.user_name}}({{item.email}})提供</v-btn>
              </v-col>
            </v-row>
          </v-card-actions>
        </v-card>
